...
> **语言选择提示**：本 skill 以 MATLAB/Octave 为上下文，但并非只能用 MATLAB。根据任务类型（见 `.claude/rules/04-tool-selection.md`），你可以选用 R 或 Python 中更合适的库。例如统计建模优先 R，ML 优先 Python，矩阵运算/优化优先 MATLAB。始终选择对当前任务最高效的工具。
