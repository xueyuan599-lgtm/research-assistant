...
## 与本仓库其它绘图 Skill 的分工（选型路由）

| 需求 | 选哪个 |
|------|--------|
| 期刊投稿图（多面板/统计标注/色盲安全，**默认**） | **本 skill（scientific-visualization，meta）** |
| 纯 Nature / SciencePlots 风格快速出图 | `nature-figure` |
| Web / 交互 / 大屏（ECharts） | `pyecharts-viz` |
| 快速探索、统计分布直出 | `seaborn`（或 `matplotlib` 裸 API） |
| 数学建模国赛图表模板（MCM 团队） | `research-assistant/knowledge/mcm/templates/figures/` |

默认优先本 meta-skill；用户指名风格/平台/场景时优先对手工指定的 skill。同一次出图只加载一个绘图 skill，避免指令叠加。
