...
## 验证标准
- 前沿方向数量 **≤ 当前档位上限**（见 `.claude/rules/05-exploration-budget.md` §1）
...
## 错误处理
- 检索结果不足 → **报告实际数量并停止**，附三个出路（升档重跑 / 换关键词重跑 / 接受当前结果）。**禁止**自动扩展时间窗口或减少限制，见 `.claude/rules/05-exploration-budget.md` §4
