"""
PNO-GWO v4.0: 黏菌管网络-灰狼混合优化算法（多峰增强版）
======================================================
Physarum Network Optimizer enhanced with Grey Wolf Optimizer,
based on Phase 1 diagnosis findings.

v4.0 相对于 v3.5 的改动（基于消融实验数据）:
  [移除] CG-PSR 种群缩减默认关闭 — 损害多数函数性能（Sphere 280×, Griewank 345×）
  [调整] FES 分割比从 80% 降到 70% — 多峰函数需要更长探索阶段（Rastrigin +14%）
  [调整] 重启阈值从 10 增到 15 — 减少无效重启（Rastrigin +5%, Ackley +73%）
  [调整] CAS H_target 下降速率从 0.5 降到 0.3 — 防止过早收敛
...
class ExternalArchive:
    ...
    def sample(self, current_pos, cond_scale, n_dim, bounds):
        ...
        # 计算电导率调制的距离权重
        lb, ub = bounds
        # 边界可能是标量（基准）或数组（工程问题），统一转标量
        span_vec = ub - lb
        span = float(np.max(np.abs(span_vec))) if np.ndim(span_vec) > 0 else float(span_vec)
        if span < 1e-10:
            span = 1.0
...
class PhysarumNetworkOptimizer:
    ...
    use_strategy_pool : bool
        是否启用多策略选择 (GWO/PNO 自适应切换, 默认开启; 关闭则强制纯 PNO)
    use_pbest_de : bool
        v4.1: 是否启用 SHADE 式 pbest-DE 臂 (current-to-pbest/1 + 二项交叉,
        针对 CEC 2017 旋转/混合函数的弱势补充, Phase 5B 实测驱动, 默认开启)
    p_de : float
        v4.1: PNO 探索臂内触发 DE 臂的概率 (默认 0.3;
        停滞门控版 (p_de=0.6, 仅停滞节点) 聚焦验证同种子下劣于无差别版——
        以停滞个体为变异基向量会强化陷阱, 故采无差别触发)
    alpha : float
        管壁生长率默认初始值 (M1 关闭时固定)
    beta : float
        管道衰减率默认初始值 (M1 关闭时固定)
    ...
    **kwargs
    ...
    def __init__(self, n_pop=None, n_dim=30, bounds=(-100, 100), max_fes=30000,
                 use_shca=True, use_cgpsr=False, use_cas=True, use_cauchy=True,
                 use_cmea=False, use_nm=True, use_twophase=True,
                 use_strategy_pool=True, use_pbest_de=True, p_de=0.3,
        ...
        self.name = kwargs.get('name', 'PNO-GWO-v4.0')
        ...
        # v4.1: SHADE 式 pbest-DE 臂（Phase 5B CEC 旋转/混合函数弱势的对证改进）
        self.use_pbest_de = use_pbest_de
        self.p_de = p_de
        ...
        # Cauchy 重启参数 (v4.0: 从 10 增到 15，减少无效重启)
        self.restart_interval = kwargs.get('restart_interval', 15)

        # 两阶段 FES 比例 (v4.0: 从 0.8 降到 0.7，多峰函数需要更长探索)
        self.phase1_ratio = kwargs.get('phase1_ratio', 0.7)
    ...
    def _reset_state(self):
        ...
        # v4.1: SHADE 式 F/CR 成功历史记忆（DE 臂）
        self.de_F_mem = np.full(5, 0.5)
        self.de_CR_mem = np.full(5, 0.5)
        self.de_mem_idx = 0
        self._succ_f = []
        self._succ_cr = []
        self._succ_imp_de = []
    ...
    def _compute_p_gwo(self, t, T, cond_entropy):
        ...
        # CAS: H_target 从 0.8 线性下降到 0.5 (v4.0: 速率从 0.5 降到 0.3，防止过早收敛)
        H_target = 0.8 - 0.3 * min(t / T, 1.0)
        k_steep = 4.0
    ...
    def _strategy_pbest_de(self, i, X, fitness, archive, F, CR):
        """策略 5 (v4.1): SHADE 式 current-to-pbest/1 + 二项交叉

        Phase 5B (CEC 2017) 实测显示旋转/混合函数上 DE 族算子
        (SHADE/jSO/CMODE, 排名 1-3) 显著优于坐标轴对齐的 GWO/PNO 移动
        (F27 落入陷阱、F10-F18 落后 100-1000×)。
        本策略作为第三臂补充旋转鲁棒的搜索方向。

        pbest 取前 10% 最优 (jSO 惯例), r2 有一半概率取自外部归档
        (JADE 惯例), 二项交叉保证至少一个维度继承变异。
        """
        n = len(X)
        n_pbest = max(2, int(n * 0.1))
        pbest_idx = np.argsort(fitness)[:n_pbest]
        x_pbest = X[pbest_idx[np.random.randint(n_pbest)]]

        r1 = np.random.randint(n)
        while r1 == i:
            r1 = np.random.randint(n)
        if archive is not None and len(archive.solutions) > 0 and np.random.rand() < 0.5:
            r2x = archive.solutions[np.random.randint(len(archive.solutions))]
        else:
        ...
        mutant = X[i] + F * (x_pbest - X[i]) + F * (X[r1] - r2x)

        # 二项交叉: jrand 强制至少一维来自变异向量
        jrand = np.random.randint(self.n_dim)
        mask = np.random.rand(self.n_dim) < CR
        mask[jrand] = True
        return np.where(mask, mutant, X[i])
    ...
    def optimize(self, obj_func, n_dim=None, bounds=None, max_fes=None, verbose=True):
        ...
        for t in range(T):
            ...
            # v4.1: 按 F/CR 成功历史逐代采样 DE 臂参数 (SHADE 惯例:
            # F~Cauchy(mem, 0.1) 截断 [0.1,1], CR~Normal(mem, 0.1) 截断 [0,1])
            if self.use_pbest_de and self.use_strategy_pool and not fes_phase2:
                fdi = self.de_mem_idx % len(self.de_F_mem)
                F_samples = np.clip(
                    self.de_F_mem[fdi] + 0.1 * np.random.standard_cauchy(n_node_now),
                    0.1, 1.0,
                )
                CR_samples = np.clip(
                    self.de_CR_mem[fdi] + 0.1 * np.random.randn(n_node_now),
                    0.0, 1.0,
                )
            else:
                F_samples = None
            ...
            for i in range(n_node_now):
                ...
                elif F_samples is not None and np.random.rand() < self.p_de:
                    # v4.1: PNO 探索臂内切换到 SHADE 式 pbest-DE 臂
                    # (Phase 5B CEC 旋转/混合函数弱势的针对性改进;
                    #  停滞门控版聚焦验证同种子下劣于无差别版, 故取消门控)
                    use_de = True
                    candidate = self._strategy_pbest_de(
                        i, X, fitness,
                        self.archive if self.use_cmea else None,
                        F_samples[i], CR_samples[i],
                    )
                else:
                    # PNO 图探索
                ...
                # v4.1: 收集 DE 臂的成功 F/CR（成功历史更新用）
                if use_de and improved:
                    self._succ_f.append(float(F_samples[i]))
                    self._succ_cr.append(float(CR_samples[i]))
                    self._succ_imp_de.append(fitness[i] - fit_candidate)
            ...
            # v4.1: SHADE 式 F/CR 成功历史更新（加权 Lehmer 均值）
            if len(self._succ_f) > 0:
                w = np.clip(np.array(self._succ_imp_de), 1e-12, None)
                den_f = np.sum(w * np.array(self._succ_f))
                den_cr = np.sum(w * np.array(self._succ_cr))
                if den_f > 1e-9:
                    self.de_F_mem[self.de_mem_idx] = (
                        np.sum(w * np.square(np.array(self._succ_f))) / den_f
                    )
                if den_cr > 1e-9:
                    self.de_CR_mem[self.de_mem_idx] = (
                        np.sum(w * np.square(np.array(self._succ_cr))) / den_cr
                    )
                if den_f > 1e-9 or den_cr > 1e-9:
                    self.de_mem_idx = (self.de_mem_idx + 1) % len(self.de_F_mem)
                self._succ_f.clear()
                self._succ_cr.clear()
                self._succ_imp_de.clear()
